...
class SentimentAnalysis(nn.Module):
    def __init__(self, hidden_units, output_units, vocab_size, embedding_dim):
        ...
        #   nn.GRU returns (output, h_n); we use h_n (final hidden state) only,
        #   which is the context_vector in Fig.12-1~12-3.
        self.gru = nn.GRU(
        ...
        # sigmoid is applied after Linear — no double-application issue
        # because MeanSquaredError does not apply any activation internally.
    ...
    def forward(self, x):
        ...
        # return_sequences=False, return_state=False equivalent:
        #   h_n: (1, batch, hidden_units) <- final hidden state = context_vector
        _, h_n = self.gru(x)

        # context_vector: (batch, hidden_units)
        context_vector = h_n.squeeze(0)
